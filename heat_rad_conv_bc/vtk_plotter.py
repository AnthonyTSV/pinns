...
class VTKPlotter(_Plotter):
    def __init__(
        ...
        self.is_already_sliced = is_already_sliced
...
if __name__ == "__main__":
    # Example usage
    plotter = VTKPlotter(
        path_to_pinns=os.path.join(BASE_DIR, "outputs/silu_interior_4000_arch_256x5/validators/vtk_val.vtu"),
        path_to_vtk=os.path.join(BASE_DIR, "fem_data/temperature_solution.vtu"),
        slice_origins=[(0, 0, 0)],
        slice_normals=[(0, 0, 1)],
        array_name="Temperature",
        is_already_sliced=True
    )
    plotter()
